 ...
 */
define(['jquery', 'log', './simulator-rest-client', 'alerts', 'lodash',
        /* void libs*/ 'bootstrap', 'theme_wso2', 'jquery_ui', 'jquery_validate', 'jquery_timepicker', './templates'],
    function ($, log, Simulator, alerts, _) {
        ...
        self.init = function (config) {
            ...
            const handler = (event) => {
                ...
                if (message.command === 'updateSiddhiAppNames') {
                    var $siddhiAppSelect = $('form[data-uuid="' + self.totalSourceNum + '"] select[name="single-event-siddhi-app-name"]');
                    var siddhiAppName = $siddhiAppSelect.val();
                    sself.createSiddhiAppMap(data);
            ...
            $("#event-feed-form").on('submit', 'form.feedSimulationConfig', function () {
                if (self.currentTotalSourceNum == 1) {
                    ...
                    properties.siddhiAppName = self.siddhiAppName;
                    ...
                    $('div.sourceConfigs div.source').each(function () {
                        ...
                        var source = {};
                        source.siddhiAppName = self.siddhiAppName;
            ...
            $("#event-feed-form").on('click', '.feedSimulationConfig button.addNewSource', function () {
                if (self.siddhiAppName == null) {
                    alerts.error("Please select a Siddhi App to add a source.");
                    return;
                }
                $('.collapse').collapse();
                ...
                var $streamNameSelect = $sourceConfigBody.find('select[name="stream-name"]');
                var $siddhiAppMode = $sourceConfigs.find('div[data-name="siddhi-app-name-mode"]');
                ...
                Simulator.retrieveStreamNames(
                    ...
                    function (data) {
                        ...
                        $sourceConfigs.find('div[class="dynamicToggleFormContent"]').show();
                    },
                    function (data) {
                        log.info(data);
                    }); 
            ...
            $("#event-feed-form").on('change', 'select[name="feed-event-siddhi-app-name"]', function () {
                var $element = $(this);
...
//                                 }
            ...
            self.$eventFeedConfigTabContent.on('click', 'a i.fw-assign', function () {
                ...
                Simulator.simulationAction(
                    self.activeSimulationList[simulationName].properties.siddhiAppName,
                    simulationName,
                    "pause",
                    function (data) {
                        var message = {
            ...
            self.$eventFeedConfigTabContent.on('click', 'a[name="edit-source"]', function () {
                ...
                self.siddhiAppName = simulationConfig.properties.siddhiAppName;
                ...
                $eventFeedForm.find('select[name="feed-event-siddhi-app-name"]')
                    .val(simulationConfig.properties.siddhiAppName);
            ...
            $("#event-feed-form").on('change', '.sourceConfigs div select[name="stream-name"]', function () {
                ...
                var newPanelHeader = "";
                newPanelHeader = splitValues[0].trim() + " : " + self.siddhiAppName + " : " + streamName;
        ...
        self.bindDynamicContent = function (sourceForm, type, uniqueId) {
            ...
            if (type == "Random") {
                ...
                sourceForm.find("button[id='upload-csv-file_" + uniqueId + "']").on('click', function () {
                    ...
                    Simulator.uploadCSVFile(function (data) {
                        log.info(data);
                        self.loadCSVFileNames(self.selectedSourceNum);
                    },
                    function (data) {
        ...
        self.createConfigPanel = function (totalSourceNum, dataCollapseNum, sourceType) {
            var panel =
                ...
                '<span class="simulationHeader">{{sourceType}} Source {{totalSourceNum}} : {{siddhiAppName}}</span>' +
                '</a>' +
                '</h4>' +
                '</div>' +
            ...
            var temp3 = temp2.replaceAll('{{siddhiAppName}}', self.siddhiAppName);
            return temp3.replaceAll('{{sourceType}}', sourceType);
        ...
        var $siddhiAppSelect = $('select[name="feed-event-siddhi-app-name"]');
        var siddhiAppName = $siddhiAppSelect.val();
        Simulator.retrieveSiddhiAppNames(
            function (data) {
                self.createSiddhiAppMap(data);
        ...
        self.validateAndGetDbConfiguration = function ($sourceConfigForm) {
            ...
            if (dataSourceLocation === null || dataSourceLocation.length === 0) {
                alerts.error("Datasource location is required to test database connection")
            }
            var driverName = $sourceConfigForm.find('input[name="driver-class"]').val();
            if (driverName === null || driverName.length === 0) {
                alerts.error("Driver is required to test database connection")
            }
            ...
            if (password === null || password.length === 0) {
                alerts.error("Password is required to test database connection")
            }
        ...
        self.addAvailableFeedSimulations = function () {
            Simulator.getFeedSimulations(
                function (data) {
                    var simulations = JSON.parse(data);
        ...
        self.pollingSimulation = function () {
            if (self.$eventFeedTab.attr("aria-expanded") && self.$eventFeedTab.closest('li').hasClass('active')) {
                self.addAvailableFeedSimulations();
                setTimeout(self.pollingSimulation, 3000);
            } else {
                setTimeout(self.pollingSimulation, 3000);
            }
        };
        ...
        self.updateFeedCreationButtonAndNotification = function () {
            Simulator.retrieveSiddhiAppNames(
                ...
                function (data) {
                    alerts.error("Error in retrieving back end data " + data);
                }
            );
