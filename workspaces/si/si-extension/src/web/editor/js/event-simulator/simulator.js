 ...
 */
define(['jquery', 'log', './constants', './simulator-rest-client', 'lodash', 
        /* void libs */'bootstrap', 'theme_wso2', 'jquery_ui', 'jquery_validate', 'jquery_timepicker', './templates'],
    function ($, log, constants,Simulator, _) {
        ...
        self.init = function (config) {
            ...
            const handler = (event) => {
                ...
                if (message.command === 'updateSiddhiAppNames') {
                    var $siddhiAppSelect = $('form[data-uuid="' + self.singleEventConfigCount + '"] select[name="single-event-siddhi-app-name"]');
                    var siddhiAppName = $siddhiAppSelect.val();
                    self.createSiddhiAppMap(message.payload);
